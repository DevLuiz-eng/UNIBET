...
void bayerxceltic(int *saldo, int *vencedor, int *golA2, int *golB2, int *totalgol) {
    ...
    int golAfrc = 0;
    char timeB[99] = "Liverpool";
    char jogadorB[99] = "Salah";
    int golB = 0;
    char jogadorBfrc[99] = "Darwin Nunez";
	int golBfrc = 0;
	...
	*golB2 += placarB;
	...
	if (placarA == placarB) {
	    ...
	    *vencedor = 3;  // vitória time A (Uruguai)
	} else if (placarB > placarA) {
	    *vencedor = 4;  // vitória time B (Itália)
	} else {
	    *vencedor = 1;  // só exibir placar
	}
}
