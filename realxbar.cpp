...
void realxbar(int *saldo, int *vencedor1, int *golA1, int *golB1, int *totalgol1) {
    ...
    int placarA = 0;
    int placarB = 0;
    char timeA[99] = "Real Madrid";
    char jogadorA[99] = "Vinicius Jr";
    int golA = 0;
    char jogadorAfrc[99] = "Mbappe";
    int golAfrc = 0;
    char timeB[99] = "Barcelona";
    char jogadorB[99] = "lamine Yamal";
    int golB = 0;
    char jogadorBfrc[99] = "lewandowski";
	int golBfrc = 0;
    ...
    while (tempo < 15) { // Primeiro tempo.
        ...
        if (gol <= 7 || gol > 295){
        	...
        	placarA++;
        	sleep(2);
		} else if (gol > 7 && gol <=14 || gol >= 290 && gol <= 295){
			printf("%s no ATAQUE.\n", timeA);
        	sleep(1);
	...
	*golB1 += placarB;
	...
	if (placarA == placarB) {
	    ...
	    *vencedor1 = 3;  // vitória time A (Uruguai)
	} else if (placarB > placarA) {
	    *vencedor1 = 4;  // vitória time B (Itália)
	} else {
	    *vencedor1 = 1;  // só exibir placar
	}
}
