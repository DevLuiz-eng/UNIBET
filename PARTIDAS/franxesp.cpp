...
void franxesp(int *saldo, int *vencedor11, int *golA12,int *golB12, int *totalgol11) { //PRECISA ALTERAR
    ...
    int placarA = 0;
    int placarB = 0;
    char timeA[99] = "França"; //PRECISA ALTERAR
    char jogadorA[99] = "Dembélé"; //PRECISA ALTERAR
    int golA = 0;
    char jogadorAfrc[99] = "Mbappé"; //PRECISA ALTERAR
    int golAfrc = 0;
    char timeB[99] = "Espanha"; //PRECISA ALTERAR
    char jogadorB[99] = "Nico Willians"; //PRECISA ALTERAR
    int golB = 0;
    char jogadorBfrc[99] = "Lamine Yamal"; //PRECISA ALTERAR
	int golBfrc = 0;
...
system("cls");
	...
	*totalgol11 += mvpgol;//PRECISA ALTERAR
	*golA12 += placarA;//PRECISA ALTERAR
	*golB12 += placarB;//PRECISA ALTERAR
	
	if (placarA == placarB) {
	    *vencedor11 = 2;  // empate
	} else if (placarA > placarB) {
	    *vencedor11 = 3;  // vitória time A (Uruguai)
	} else if (placarB > placarA) {
	    *vencedor11 = 4;  // vitória time B (Itália)
	} else {
	    *vencedor11 = 1;  // só exibir placar
	}
