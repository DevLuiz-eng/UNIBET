...
void psgxars(int *saldo, int *vencedor8, int *golA9,int *golB9, int *totalgol8) { //PRECISA ALTERAR
    ...
    int placarA = 0;
    int placarB = 0;
    char timeA[99] = "PSG"; //PRECISA ALTERAR
    char jogadorA[99] = "Dembélé"; //PRECISA ALTERAR
    int golA = 0;
    char jogadorAfrc[99] = "Doué"; //PRECISA ALTERAR
    int golAfrc = 0;
    char timeB[99] = "Arsenal"; //PRECISA ALTERAR
    char jogadorB[99] = "Saka"; //PRECISA ALTERAR
    int golB = 0;
    char jogadorBfrc[99] = "Havertz"; //PRECISA ALTERAR
	int golBfrc = 0;
	...
	*totalgol8 += mvpgol;//PRECISA ALTERAR
	*golA9 += placarA;//PRECISA ALTERAR
	*golB9 += placarB;//PRECISA ALTERAR
	
	if (placarA == placarB) {
	    *vencedor8 = 2;  // empate
	} else if (placarA > placarB) {
	    *vencedor8 = 3;  // vitória time A (Uruguai)
	} else if (placarB > placarA) {
	    *vencedor8 = 4;  // vitória time B (Itália)
	} else {
	    *vencedor8 = 1;  // só exibir placar
	}
}
