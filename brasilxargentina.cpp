...
void brasilxargentina(int *saldo, int *vencedor3, int *golA4, int *golB4, int *totalgol3) { //PRECISA ALTERAR
    ...
    int placarA = 0;
    int placarB = 0;
    char timeA[99] = "Brasil"; //PRECISA ALTERAR
    char jogadorA[99] = "Neymar"; //PRECISA ALTERAR
    int golA = 0;
    char jogadorAfrc[99] = "Vinicius Jr"; //PRECISA ALTERAR
    int golAfrc = 0;
    char timeB[99] = "Argentina"; //PRECISA ALTERAR
    char jogadorB[99] = "Messi"; //PRECISA ALTERAR
    int golB = 0;
    char jogadorBfrc[99] = "Julian Alvarez"; //PRECISA ALTERAR
	int golBfrc = 0;
	...
	printf("Gols do jogo\n%s - %d gol\n%s - %d gol\n%s - %d gol\n%s - %d gol\n\n", jogadorA, golA, jogadorAfrc, golAfrc, jogadorB, golB,jogadorBfrc, golBfrc);

	//mercados de aposta
	
	*totalgol3 += mvpgol;//PRECISA ALTERAR
	*golA4 += placarA;//PRECISA ALTERAR
	*golB4 += placarB;//PRECISA ALTERAR
	
	if (placarA > placarB) {
	    *vencedor3 += 2; //PRECISA ALTERAR
	} else if (placarB > placarA) {
	    *vencedor3 += 3; //PRECISA ALTERAR
	} else if (placarA == placarB || placarB == placarA){
	    *vencedor3++; //PRECISA ALTERAR
	}
}
